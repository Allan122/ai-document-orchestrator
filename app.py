...
try:
    # THE FIX: .strip() removes any hidden spaces or newlines that cause the URL error
    N8N_WEBHOOK_URL = st.secrets["N8N_WEBHOOK_URL"].strip()
    ...
    if st.button("Send Alert Mail"):
        with st.spinner("Analyzing document and running n8n workflow..."):
            try:
                ...
                # 2. BULLETPROOF DIRECT API CALL TO GEMINI (Bypassing Streamlit's old cache)
